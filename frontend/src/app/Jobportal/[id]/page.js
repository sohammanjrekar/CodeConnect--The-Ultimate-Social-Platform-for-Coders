...
const jobcall = ({ params }) => {
  const [jobPosting, setJobPosting] = useState(null); // Initialize with null
  ...
  useEffect(() => {
    ...
    axios.get(`http://127.0.0.1:8000/jobportal/job-postings/${id}`)
      ...
      .catch(error => {
        ...
        setJobPosting([]); // Handle error by setting jobPosting to an empty array
      });
  }, []);
  ...
  return (
    <div>
      <Navbar/>
      <>
      {jobPosting && Array.isArray(jobPosting) && jobPosting.map(job => (
    <div key={job.id} className="px-4 bg-white mb-8 py-8 rounded-3xl mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8">
      <div className="flex flex-col items-center justify-between w-full mb-10 lg:flex-row">
        <div className="mb-16 lg:mb-0 lg:max-w-lg lg:pr-5">
          <div className="max-w-xl mb-6">
            ...
            <p className="text-black text-base md:text-lg">
              <strong>Application Email:</strong> {job.application_email}
            </p>
            <p className="text-black text-base md:text-lg">
              <strong>Application Deadline:</strong> {new Date(job.application_deadline).toLocaleDateString()}
            </p>
            <p className="text-black text-base md:text-lg">
              <strong>Application Tracking Link:</strong> <a href={job.application_tracking_link} className="text-blue-500 underline hover:text-blue-400">{job.application_tracking_link}</a>
            </p>
          </div>
    ...
    <li className="col-span-1 divide-y divide-gray-200 rounded-lg bg-white shadow">
      <div className="flex w-full items-center justify-between space-x-6 p-6">
        <div className="flex-1 truncate">
          ...
          <p className="mt-1 truncate text-sm text-gray-500">Web developer</p>
        </div>
        <img
          className="h-10 w-10 flex-shrink-0 rounded-full bg-gray-300"
      ...
      <div className="flex w-full items-center justify-between space-x-6 p-6">
        <div className="flex-1 truncate">
          <div className="flex items-center space-x-3">
            ...
            <span className="inline-flex flex-shrink-0 items-center rounded-full bg-green-50 px-1.5 py-0.5 text-xs font-medium text-gray-600 ring-1 ring-inset ring-green-600/20">
              leader helper
            </span>
          </div>
          <p className="mt-1 truncate text-sm text-gray-500">Master</p>
        </div>
        <img
          className="h-10 w-10 flex-shrink-0 rounded-full bg-gray-300"
    ...
    <li className="col-span-1 divide-y divide-gray-200 rounded-lg bg-white shadow">
      <div className="flex w-full items-center justify-between space-x-6 p-6">
        <div className="flex-1 truncate">
          ...
          <p className="mt-1 truncate text-sm text-gray-500">Leader</p>
        </div>
        <img
          className="h-10 w-10 flex-shrink-0 rounded-full bg-gray-300"
